...
/*
 * A code generator for import/export nodes in ES6 using Esprima's AST.
 *
...
function generateList(specifiers) {
    ...
    var firstName = getId(firstSpecifier);
    var str = "";
    var start = 1;

    if (firstType === "ImportDefaultSpecifier") {
        str = firstName;

        if (len === 1)
            return str;
        else if (specifiers[1].type === "ImportNamespaceSpecifier")
            return str + ", * as " + getId(specifiers[1]);

        str += ", ";
    } else if (firstType === "ImportNamespaceSpecifier") {
        str = "* as " + firstName;

        if (len === 1)
            return str;
        else if (specifiers[1].type === "ImportDefaultSpecifier")
            return str + ", " + getId(specifiers[1]);
